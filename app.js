...
app.post('/otp/send',async(req,res,next)=>{
    ...
    crypto.randomBytes(4,(err,buffer)=>{
       ...
       user.save() // await user.save() , await say : wait until it store successfully on db then complete my code
       res.status(201).json({
...
app.post('/otp/verify',async(req,res,next)=>{
    ...
    user.confirmed= "true" // hey lato , true not 'true' , because it boolean not string
    await user.save()
    res.status(200).json({
        message:"the email confirmed successfully ya 3aaam"
    })
    /*
        you have to put catch here y wad , like previous function 
    */
})

app.post('/signup',async(req,res,next)=>{
    // good work bro , but must you seprate these models 
    // model for email , model for user , in user model we have email attribute which has full information in email model 
    try{
    const email=req.body.email;
...
app.post('/forget-password/verify',async(req,res,next)=>{
    ...
    const password=req.body.password;
    const user=await User.findOne({email,verify_code,confirmed:"false"}) // confirmed:false , not 'false' y wad
    if(!user){
            return res.status(404).json({
                message:"the email is not founded"
            })
...
app.post('/change-password',async(req,res,next)=>{ // beutiful :)
    try{
    const email=req.body.email;
    const password=req.body.password;
